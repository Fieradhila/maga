...
for ($i=2; $i<=$jumlah_baris; $i++){
	...
	if($NIK != "" && $ID != "" && $nama_krywn != "" && $tgl_lahir != "" && $status_nikah != "" && $alamat != "" && $gender != "" && $pendidikan != "" && $agama != "" && $no_hp != "" && $tgl_masuk != "" && $jabatan != "" && $cabang != "" && $departement != "" && $email != "" && $status_kerja != "" && $status_aktif != ""){
		...
		('$NIK','$ID','$nama_krywn','$tgl_lahir','$status_nikah','$alamat','$gender','$pendidikan','$agama','$no_hp','$tgl_masuk','$jabatan','$cabang','$departement','$email','$status_kerja','$status_aktif', '$namefilefoto','$namefilektp', '$namefilekk', '$namefilenikah')");

		$berhasil++;

		//header("location:daftar_krywn.php?berhasil");
	}
}
